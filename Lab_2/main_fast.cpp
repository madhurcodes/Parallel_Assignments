...
vector<int> iter_refine1(vector<int> lbl,vector<vector<pair<int,int> > > &adj)
{
    ...
    if(z_cnt>o_cnt)
    {
        ...
        int diff = int(0.85*(z_cnt - o_cnt)/2);
        for(i=0;i<diff;i++)
            lbl[d_0[i].second] = 1;
    }
    else if(z_cnt < o_cnt)
    {
        ...
        int diff = int(0.85*(o_cnt - z_cnt)/2);
        for(i=0;i<diff;i++)
            lbl[d_1[i].second] = 0;
    }
...
vector<int> iter_refine(vector<int> lbl,vector<vector<pair<int,int> > > &adj)
{
    ...
    while(cnt<50)
    {
        ...
        int cng_lbl = -1;
        int cng_val;
        if((z_cnt>o_cnt) || (static_cast <float> (rand()) / static_cast <float> (RAND_MAX) < 0.0005))
        {
            cng_val = 1;
            for(i=1;i<n;i++)
            {
        ...
        else if((z_cnt<o_cnt)|| (static_cast <float> (rand()) / static_cast <float> (RAND_MAX) < 0.0005))
        {
            cng_val = 0;
            for(i=1;i<n;i++)
            {
        ...
        else if(static_cast <float> (rand()) / static_cast <float> (RAND_MAX) < 0.0001){
            cout << "eq conts "<<endl;
            break;
        }
        if(mx_cng <= 0){
            ...
            cnt = 0;
            d[cng_lbl] = -100;
            for(i=0;i<adj[cng_lbl].size();i++){
                int v = adj[cng_lbl][i].first;
                int x = adj[cng_lbl][i].second;
                if(cng_val==lbl[v])
                {
                    d[v] = d[v] - x;
                    // d[v] = d[v] - x;
...
vector<int> fullbisect(vector <vector <pair<int,int> > >  &adj)
{
    ...
    while(ret1.first.size()>500 && (step<50))
    {
...
{   
    ...
    cout<<time_taken<<" seconds taken"<<endl;
    return 0;
}
