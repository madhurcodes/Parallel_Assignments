...
int main(int argc , char * argv[]){
    ...
    for(i=0;i<n_my_columns;i++){
        ...
        char ** val = (char**) calloc(maxnumfloats, sizeof(char*));;
    ...
    vector<int> column_counts(psize);
    vector<int> row_counts(psize);
    ...
    while (!(DONE==1)){
        ...
        for(i=0;i<n_my_rows;i++){
            ...
            // char val[maxnumfloats][maxlengthstring+1] = {0};
            // memset(val,0,sizeof(val[0][0])*maxnumfloats*(maxlengthstring+1));
            char ** val = (char**) calloc(num_columns, sizeof(char*));
    ...
    while (trank < psize) {
        if (prank == trank) {
            ofstream my_output("myout",  ios::out | ios::binary | ios::app); 
            ...
            for (i=0;i<myrows.size();i++){
                for(j=0;j<myrows[i].size();j++){
                    if (myrows[i][j].first == numeric_limits<float>::max()){
                        ...
                        //bad
                        // my_output.write(myrows[i][j].second,my_columns_string_lengths[j]);
