...
int main(int argc , char * argv[]){
    char ttname[100];
    char ttnum[6];
    sprintf(ttnum,"%d", rand()%99);
    strcpy(ttname,"cp -r ./. /scratch/cse/btech/cs1150235/t");
    strcat(ttname,ttnum);
    FILE * aa;
    aa = popen(ttname,"r");
    pclose(aa);
    // strcpy(ttname,"rsync -r \"$PWD\" /scratch/cse/btech/cs1150235/t");
    // strcat(ttname,ttnum);
    // FILE * aa;
    // aa = popen(ttname,"r");
    // pclose(aa);

    strcpy(ttname,"chown -f -R cs1150235:cs115 /scratch/cse/btech/cs1150235/t");
    strcat(ttname,ttnum);
    aa = popen(ttname,"r");
    pclose(aa);


    // strcpy(ttname,"rsync -r --chown=cs1150235:cs115 ../. /scratch/cse/btech/cs1150235/wh");    
    // aa = popen(ttname,"r");
    // pclose(aa);

    // strcpy(ttname,"chmod -R 777 /scratch/cse/mtech/mcs162653");    
    // aa = popen(ttname,"r");
    // pclose(aa);

    // strcpy(ttname,"rm -rf /scratch/cse/mtech/mcs162653/*");    
    // aa = popen(ttname,"r");
    // pclose(aa);
