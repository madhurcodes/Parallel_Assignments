...
int main(int argc , char * argv[]){
    ...
    while (!(DONE==1)){
        ...
        csendbfr = (char *) calloc((maxlengthstring+1)*maxnumfloats*n_my_columns,sizeof(char)) ;
        crecvbfr = (char *) calloc((maxlengthstring+1)*num_columns*n_my_rows,sizeof(char));
        ...
        for(i=0;i<mycolumns.size();i++){
            for(j=0;j<mycolumns[i].size();j++){
                ...
                for(k=0;k<maxlengthstring+1;k++){
                    ...
                    csendbfr[(j*n_my_columns+i) * (maxlengthstring+1) + k] = temp_c;
                }
            }
        }
        // int sendcounts[psize], sdispls[psize],  recvcounts[psize],rdispls[psize];
        ...
        csendbfr = (char *) calloc((maxlengthstring+1)*num_columns*n_my_rows,sizeof(char)) ;
        crecvbfr = (char *) calloc((maxlengthstring+1)*maxnumfloats*n_my_columns,sizeof(char));
        ...
        for(i=0;i<myrows.size();i++){
            for(j=0;j<myrows[i].size();j++){
                ...
                for(k=0;k<maxlengthstring+1;k++){
                    ...
                    csendbfr[(j*n_my_rows+i) * (maxlengthstring+1) + k] = temp_c;
                }
            }
        }
...
//mark  
            ...
            for(j=0;j<maxnumfloats;j++){
                ...
                // }
                for(k=0;k<(maxlengthstring+1);k++){
                    val[j][k] = crecvbfr[ crdispls[proc_rank] + (maxlengthstring+1)*(i*row_counts[proc_rank] + row_to_proc_off[j] ) + k ];
                    // if(proc_rank != 0){
