...
int main(int argc , char * argv[]){
    ...
    for(num_it=0;num_it<4;num_it++){
        ...
        //sort rows
        vector <pair<float, pair<int,int> > > tosort_row;
        vector <vector<int> > newrow;
        vector<int> newel_row;
        #pragma omp parallel for shared(list_of_rows,col_indices_in_rows) private(tosort_row,newel_row,newrow,j,k)
        for (i=0;i<list_of_rows.size();i++){
            // argsort one row according to float
            tosort_row = vector <pair<float, pair<int,int> > >();
            for(j=0;j<list_of_rows[i].size();j++){
                tosort_row.push_back(make_pair(*reinterpret_cast<float *>( &list_of_rows[i][j][3] ), make_pair(list_of_rows[i][j][1],j)));
            }
            sort(tosort_row.begin(),tosort_row.end());
            newrow = vector <vector<int> >();
            for(j=0;j<list_of_rows[i].size();j++){
                newel_row = vector<int>();
                newel_row.push_back(list_of_rows[i][tosort_row[j].second.second][0]);
                newel_row.push_back(col_indices_in_rows[i][j]);
                newel_row.push_back(list_of_rows[i][tosort_row[j].second.second][2]);
                newel_row.push_back(list_of_rows[i][tosort_row[j].second.second][3]);
                newrow.push_back(newel_row);
            }
            for(j=0;j<list_of_rows[i].size();j++){
                for(k=0;k<4;k++){
        ...
        vector <pair<float, pair<int,int> > > tosort_col;
        vector <vector<int> > newcol;
        vector<int> newel_col;
        #pragma omp parallel for shared(list_of_cols,row_indices_in_cols) private(tosort_col,newel_col,newcol,j,k)        
        for (i=0;i<list_of_cols.size();i++){
            // argsort one col according to float
            tosort_col =  vector <pair<float, pair<int,int> > > ();
            for(j=0;j<list_of_cols[i].size();j++){
                tosort_col.push_back(make_pair(list_of_cols[i][j][2] ,make_pair(list_of_cols[i][j][0],j)));
            }
            sort(tosort_col.begin(),tosort_col.end());
            newcol = vector <vector<int> >();
            for(j=0;j<list_of_cols[i].size();j++){
                newel_col = vector<int>();
                newel_col.push_back(row_indices_in_cols[i][j]);
                newel_col.push_back(list_of_cols[i][tosort_col[j].second.second][1]);
                newel_col.push_back(list_of_cols[i][tosort_col[j].second.second][2]);
                newel_col.push_back(list_of_cols[i][tosort_col[j].second.second][3]);
                newcol.push_back(newel_col);
            }
            for(j=0;j<list_of_cols[i].size();j++){
                for(k=0;k<4;k++){
